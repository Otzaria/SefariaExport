...
class DeadLetterReportingTest(unittest.TestCase):
    ...
    def test_a_scan_with_no_published_intents_still_reports_a_stale_ack(self):
        """A scheduled tick can find nothing to scan — every release aged out of
        the window, or the intent assets were pruned.  Nothing can match a
        tombstone there, so the store must still be reported instead of rotting
        behind one unchanging line for as long as the window stays empty."""
        never_published = make_release(29999999999).tag
        code, out, err = self.scan(
            [], [], {never_published: acknowledgement(tag=never_published)}
        )
        self.assertEqual(0, code)
        self.assertIn("no published downstream intents", out)
        self.assertIn(
            f"::warning::stale acknowledgement: {never_published} (root {ACKNOWLEDGED_ROOT}, "
            "acked 2026-09-07) matched no terminal dead letter in this scan; "
            "update or drop it in acknowledged_dead_letters.json",
            out,
        )
        self.assertNotIn("::notice::", out)
        # There is no scan to summarise on this path, so the summary line the
        # full scan always prints must not appear here.
        self.assertNotIn("downstream reconciliation summary", out)
        self.assertEqual("", err)

    def test_a_scan_with_no_published_intents_and_an_empty_store_stays_quiet(self):
        """With nothing scanned every entry is unmatched by definition, so an
        empty store is the only quiet case — and it has to stay byte-for-byte
        as quiet as it was before this path learned to report, keeping the
        workflow's annotation count at zero."""
        code, out, err = self.scan([], [], {})
        self.assertEqual(0, code)
        self.assertEqual("no published downstream intents\n", out)
        self.assertEqual("", err)

    def test_an_exact_tag_run_never_reports_a_stale_acknowledgement(self):
        """published_intents already raises when a --tag matches no published
        intent, so this mirrors the full-scan gate defensively: an operator
        asking about one tag is never told to edit the store."""
        code, out, err = self.scan(
            [],
            [],
            {ACKNOWLEDGED_TAG: acknowledgement()},
            argv=["--tag", ACKNOWLEDGED_TAG],
        )
        self.assertEqual(0, code)
        self.assertEqual("no published downstream intents\n", out)
        self.assertEqual("", err)
