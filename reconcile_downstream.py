...
def report_stale_acknowledgements(
    acknowledgements: dict[str, DeadLetterAcknowledgement],
    acknowledged: list[DeadLetterAcknowledgement],
) -> None:
    """Warn about every tombstone this scan could not match.

    Separate from ``report_scan`` because a scan with no published intents at
    all still owns the store (every entry is unmatched there by definition) but
    has nothing to summarise, and an empty scan must stay as quiet as it is
    today when no acknowledgement is stale.
    """
...
def report_scan(
    ...
    if not failures:
        ...
        report_stale_acknowledgements(acknowledgements, acknowledged)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not releases:
            ...
            # Nothing to match means every tombstone is unmatched, so a store
            # that has outlived its releases still has to be reported instead
            # of rotting on the common scheduled tick.  No summary line here:
            # there is no scan to summarise, and a store with nothing stale in
            # it must leave this path exactly as quiet as it was.  Gated like
            # the full-scan report below; an unmatched --tag never reaches here
            # anyway, because published_intents raises on it.
            if not args.tag:
                report_stale_acknowledgements(acknowledgements, [])
            return 0
        runs = target_runs()
        failures = []
        counts: dict[str, int] = {}
